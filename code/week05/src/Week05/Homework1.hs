...
{- | This policy should only allow minting (or burning) of tokens
 if the owner of the specified PubKeyHash has signed the
 transaction and if the specified deadline has not passed.
-}
mkPolicy :: PubKeyHash -> POSIXTime -> () -> ScriptContext -> Bool
mkPolicy pkh deadline () ctx =
  txSignedBy txInfo pkh
    && after deadline vRange
  where
    txInfo :: TxInfo
    txInfo = scriptContextTxInfo ctx
    vRange :: POSIXTimeRange
    vRange = txInfoValidRange txInfo

policy :: PubKeyHash -> POSIXTime -> Scripts.MintingPolicy
policy pkh deadline =
  mkMintingPolicyScript $
    $$( PlutusTx.compile
          [||\p d -> Scripts.wrapMintingPolicy $ mkPolicy p d||]
      )
