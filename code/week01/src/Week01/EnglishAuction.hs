...
mkAuctionValidator ad redeemer ctx =
          ...
          && case adHighestBid ad of
            Nothing ->
              traceIfFalse
                ...
                (getsValue (aSeller auction) tokenValue)
            Just Bid {..} ->
              traceIfFalse
                "expected highest bidder to get token"
                (getsValue bBidder tokenValue)
                && traceIfFalse
  ...
  where
    ...
    correctCloseSlotRange =
      from (aDeadline auction) `contains` txInfoValidRange info

    getsValue :: PubKeyHash -> Value -> Bool
    getsValue h v =
      let [o] = [o' | o' <- txInfoOutputs info, txOutValue o' == v]
