...
-- This should validate if either beneficiary1 has signed the
-- transaction and the current slot is before or at the deadline
-- or if beneficiary2 has signed the transaction and the deadline
-- has passed.
mkValidator :: VestingDatum -> () -> ScriptContext -> Bool
mkValidator vd _ ctx = validBeneficiary
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx
    deadlineReached :: Bool
    deadlineReached = contains (from $ deadline vd) $ txInfoValidRange info
    validBeneficiary :: Bool
    validBeneficiary = txSignedBy info ben
      where
        ben
          | deadlineReached = beneficiary2 vd
          | otherwise = beneficiary1 vd
